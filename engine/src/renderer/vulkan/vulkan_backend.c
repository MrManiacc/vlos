...
void regenerate_framebuffers(renderer_backend *backend, vulkan_swapchain *swapchain, vulkan_render_pass *renderpass);
...
void vulkan_renderer_backend_on_resized(renderer_backend *backend, u16 width, u16 height) {
    ...
    vinfo("Vulkan renderer backend->resized: w/h/gen: %i/%i/%llu", width, height, context.framebuffer_size_generation);
}
...
b8 vulkan_renderer_backend_begin_frame(renderer_backend *backend, f32 delta_time) {
    ...
    if (context.recreating_swapchain) {
        ...
        if (!vulkan_result_is_success(result)) {
            verror("vulkan_renderer_backend_begin_frame vkDeviceWaitIdle (1) failed: '%s'",
                   vulkan_result_string(result, true));
            return false;
        }
        vinfo("Recreating swapchain, booting.");
        return false;
    }
    ...
    if (context.framebuffer_size_generation != context.framebuffer_size_last_generation) {
        VkResult result = vkDeviceWaitIdle(device->logical_device);
        if (!vulkan_result_is_success(result)) {
            verror("vulkan_renderer_backend_begin_frame vkDeviceWaitIdle (2) failed: '%s'",
                   vulkan_result_string(result, true));
            return false;
        }
    ...
    if (!vulkan_fence_wait(
            ...
            UINT64_MAX)) {
        vwarn("In-flight fence wait failure!");
        return false;
    }
...
b8 vulkan_renderer_backend_end_frame(renderer_backend *backend, f32 delta_time) {
    ...
    if (result != VK_SUCCESS) {
        verror("vkQueueSubmit failed with result: %s", vulkan_result_string(result, true));
        return false;
    }
...
VKAPI_ATTR VkBool32 VKAPI_CALL vk_debug_callback(
    ...
    switch (message_severity) {
        ...
        case VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT:
            verror(callback_data->pMessage);
            break;
        case VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT:
            vwarn(callback_data->pMessage);
            break;
        case VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT:
            vinfo(callback_data->pMessage);
            break;
        case VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT:
            vtrace(callback_data->pMessage);
            break;
    }
    return VK_FALSE;
}
...
b8 recreate_swapchain(renderer_backend *backend) {
    ...
    if (context.framebuffer_width == 0 || context.framebuffer_height == 0) {
        vdebug("recreate_swapchain called when window is < 1 in a dimension. Booting.");
        return false;
    }
