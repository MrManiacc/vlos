...
void vulkan_framebuffer_create(
    ...
    out_framebuffer->attachments = mem_alloc(sizeof(VkImageView) * attachment_count, MEM_TAG_RENDERER);
...
void vulkan_framebuffer_destroy(vulkan_context *context, vulkan_framebuffer *framebuffer) {
    ...
    if (framebuffer->attachments) {
        mem_free(framebuffer->attachments, sizeof(VkImageView) * framebuffer->attachment_count, MEM_TAG_RENDERER);
