...
b8 vulkan_swapchain_acquire_next_image_index(
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        ...
        vfatal("Failed to acquire swapchain image, message: %s", vulkan_result_string(result, true));
        return false;
    }
...
void create(vulkan_context *context, u32 width, u32 height, vulkan_swapchain *swapchain) {
    ...
    if (!swapchain->image_views) {
        swapchain->image_views = (VkImageView *) mem_alloc(sizeof(VkImageView) * swapchain->image_count,
                                                           MEM_TAG_RENDERER);
    }
